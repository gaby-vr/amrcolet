@props(['curieri', 'disounts' => [], 'prices' => [], 'input' => true, 'title' => null])

@if($title !== null && $curieri->isNotEmpty())
    <div class="col p-0">
        <h6 class="px-4"><b>{{ __($title) }}</b></h6>
@endif

@forelse($curieri ?? [] as $curier)
<div class="input-field col l12 m6 s12 mb-1 mt-0">
    <div class="card m-0 @if(!$input) opacity-75 @endif">
        <div class="card-content pt-1 pb-1 m-0">
            <div class="row mb-0">
                <div class="input-field col l1 m12 s12 items-center flex p-0 @if($input) justify-content-between @else justify-content-center @endif" style="min-width: 105px;">
                    @if($input)
                        <div class="inline-flex curier m-2">
                    ...
                    <img class="inline-block" style="width: 70px;" src="{{ asset('img/curieri/'.$curier->logo ) }}">
                </div>
                <div class="input-field col l1 m12 s12 text-center p-0 relative" style="min-width: 135px;">
                	@if($discounts[$curier->id])
                	<p class="absolute font-weight-700 text-red-500" style="top: -22px; left: 0;
