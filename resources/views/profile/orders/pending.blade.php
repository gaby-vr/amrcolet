@php
    $publicCurieri = App\Models\Curier::where('type', 1)->get();
    $userCurieri = Auth::user()?->couriers ?? collect();

    $curieri = $publicCurieri->merge($userCurieri)->unique('id')->values();

    $defaultFavorit = 'DPD';
    $curierFavorit = $curieri[0];
    foreach ($curieri as $curier) {
        if ($curier->name == $defaultFavorit) {
            $curierFavorit = $curier;
            break;
        }
    }
@endphp
...
<x-jet-table>
    ...
    <div class="input-field col-span-12 sm:col-span-6 md:col-span-2 mb-0">
        <button type="submit" class="btn blue darken-2 waves-effect waves-light">{{ __('Salveaza') }}</button>
    </div>
</x-jet-table>
<x-jet-table>
        ...
        <x-jet-td>
            <div class="input-field col-span-12 sm:col-span-6 md:col-span-4 mb-0">
                <select class="browser-default" id="status" name="status" >
                    ...
                    @foreach($curieri as $curier)
                        <option value="{{ $curier->name }}" @selected(isset($condtitions['status']) && $condtitions['status'] == $status)>
                            {{ $curier->name }}
                        </option>
                    @endforeach
                </select>
