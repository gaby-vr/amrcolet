<x-jet-table>
    <h5 class="col-span-12 mt-2"><i>{{ __('Tabel sabloane') }}</i></h5>
    <x-slot name="thead">
        @forelse($templates as $template)
        <x-jet-tr location='thead'>
            <x-jet-td location='thead'>{{ __('Nume') }}</x-jet-td>
            <x-jet-td location='thead'>{{ __('Tip Pachet') }}</x-jet-td>
            <x-jet-td location='thead'>{{ __('Nr. Colete') }}</x-jet-td>
            <x-jet-td location='thead' class="text-center">{{ __('Favorit') }}</x-jet-td>
    ...
    @forelse($templates as $template)
    <x-jet-tr>
        <x-jet-td>{{ $template->name }}</x-jet-td>
        <x-jet-td>{{ $template->type == '1' ? 'Colet' : 'Plic' }}</x-jet-td>
        <x-jet-td>{{ $template->type == '1' ? $template->nr_colete : '/' }}</x-jet-td>
        <x-jet-td class="sm:text-center">{!! $template->favorite == '1' ? '<i class="fas fa-star text-yellow-500"></i>' : '<i class="far fa-star text-yellow-500"></i>' !!}</x-jet-td>
        <x-jet-td>
            <a href="javascript:void(0)" class="btn-form-edit" data-get="{{ route('dashboard.templates.get', ['template' => $template->id]) }}" data-route="{{ route('dashboard.templates.update', ['template' => $template->id]) }}" title="{{ __('Edit') }}">
                <i class="fas fa-pencil-alt text-blue-500"></i>
            </a>
            /
            <form method="POST" action="{{ route('dashboard.templates.delete', ['template' => $template->id]) }}" class="inline" title="{{ __('Delete') }}">
                @csrf
                <a href="{{ route('dashboard.templates.delete', ['template' => $template->id]) }}"
    ...
    <x-jet-tr class="border-0" style="height: 100%;">
        <x-jet-td colspan="5" class="rounded-r-md text-center" style="height: 100%; margin-bottom: 1.25px;">{{ __('Nu ai salvat nici un sablon') }}</x-jet-td>
    </x-jet-tr>
    @endif
    ...
    <x-slot name="form">
        ...
        <div class="row col-span-12 m-0">
            ...
            <div class="input-field col m2 s12 show-colet">
                <x-jet-label for="nr_colete" class="active">Numar colete <span class="red-text">*</span></x-jet-label>
            ...
            <div class="input-field col m4 s12">
                <x-jet-label for="content" class="active">Continut <span class="red-text">*</span></x-jet-label>
                ...
                <div class="card m-0">
                    <div class="card-content pt-2 pb-2 m-0">
                        <p class="inline-block">
                            ...
                            <span class="total-volume" data-name="total_volume">0</span> kg
                        </p>
                    </div>
                </div>
            </div>
